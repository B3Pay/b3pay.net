...
const KIND: Record<WorkflowNodeKind, { accent: string; strong: string }> = {
  ...
  return (
    ...
    >
      <div className="b3-node__head">
        <span className="b3-node__glyph">
          {Icon ? (
            <Icon width={12} height={12} aria-hidden />
          ) : (
            <span style={{ fontSize: 9, fontFamily: "var(--font-mono)" }} aria-hidden>
              {kind[0].toUpperCase()}
            </span>
          )}
        </span>
        ...
        <span
          style={{ fontFamily: "var(--font-mono)", fontSize: 9, color: "var(--node-accent)" }}
        >
          $N{index}
        </span>
      </div>
      ...
      {ports ? (
        <>
          <span
            ...
            style={{ position: "absolute", left: -5, top: "50%", marginTop: -4 }}
          />
          <span
            className="b3-node__port"
            style={{ position: "absolute", right: -5, top: "50%", marginTop: -4 }}
          />
        </>
      ) : null}
    </div>
